...
void main()
{
    ...
    if(glH_LightingEnabled != 0)
    {
        ...
        // Normal map
        if(has_normal_map)
            nN = HOUapplyNormalMap(p, nN, fsIn.texcoord0, false,
                                   vec3(0.0), vec3(0.0));
...
if (MAT(has_env_map) || glH_HasSceneEnvMap)
{
    ...
    ldiff += tex.rgb * alpha * MAT(diffuse_color) * amb * mtl1 * mix(DF, 1.0, mtl);
